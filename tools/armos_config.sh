...
BUILD_NEWLIB BUILD_ALL_USERLAND BUILD_TCC BUILD_BSD BUILD_NCURSES BUILD_LIBEDIT BUILD_FREEBSD_SH BUILD_FREEBSD_TOP BUILD_NANO BUILD_EPOLL_SHIM BUILD_PIXMAN BUILD_TLLIST BUILD_UTF8PROC BUILD_FREETYPE BUILD_EXPAT BUILD_FONTCONFIG BUILD_HARFBUZZ BUILD_FCFT BUILD_FOOT BUILD_NUKLEAR BUILD_MESA BUILD_RAYLIB
...
armos_config_validate() {
    ...
    if [ "${BUILD_LIBEDIT:-0}" = 1 ] && [ "${BUILD_NCURSES:-0}" = 0 ] &&
       [ ! -f "$root_dir/build/${TARGET_ARCH:-arm32}/${TARGET_PLATFORM:-qemu-virt}/userfs/opt/ncurses/lib/libncurses.a" ]; then
        armos_config_error "BUILD_LIBEDIT=yes requires BUILD_NCURSES=yes or an installed ncurses bundle"
        return 1
    fi

    if [ "${BUILD_FREEBSD_SH:-0}" = 1 ] && [ "${BUILD_LIBEDIT:-0}" = 0 ] &&
       [ ! -f "$root_dir/build/${TARGET_ARCH:-arm32}/${TARGET_PLATFORM:-qemu-virt}/userfs/opt/libedit/lib/libedit.a" ]; then
        armos_config_error "BUILD_FREEBSD_SH=yes requires BUILD_LIBEDIT=yes or an installed libedit bundle"
        return 1
    fi

    if [ "${BUILD_FREEBSD_TOP:-0}" = 1 ] && [ "${BUILD_NCURSES:-0}" = 0 ] &&
       [ ! -f "$root_dir/build/${TARGET_ARCH:-arm32}/${TARGET_PLATFORM:-qemu-virt}/userfs/opt/ncurses/lib/libncurses.a" ]; then
        armos_config_error "BUILD_FREEBSD_TOP=yes requires BUILD_NCURSES=yes or an installed ncurses bundle"
        return 1
    fi

    if [ "${BUILD_MESA:-0}" = 1 ] &&
       { [ "${TARGET_ARCH:-arm32}" != arm64 ] ||
         [ "${TARGET_PLATFORM:-qemu-virt}" != qemu-virt ]; }; then
        armos_config_error "BUILD_MESA=yes currently requires arm64/qemu-virt"
        return 1
    fi

    if [ "${BUILD_RAYLIB:-0}" = 1 ] && [ "${BUILD_MESA:-0}" = 0 ]; then
        armos_config_error "BUILD_RAYLIB=yes requires BUILD_MESA=yes"
        return 1
    fi
}
