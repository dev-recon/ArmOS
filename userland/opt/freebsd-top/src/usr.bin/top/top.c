...
main(int argc, const char *argv[])
{
	...
	if (preset_argc == 0)
	{
	    ...
	    /*
	     * Newlib resets both optind and its private optwhere state only when
	     * optind is zero.  FreeBSD libc accepts a reset to one, but doing so
	     * here makes the first short option start in argv[0] on ArmOS.
	     */
	    optind = 0;
	}
